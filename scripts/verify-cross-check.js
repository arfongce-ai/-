...
assertContains(webHtml, "태권도 품새 수련인을 위한 영상 기반 AI 분석 코치 · v3.87", "헤더 설명 문구 적용(버전 표시는 배포마다 갱신 필요)");
...
assertContains(webHtml, "쉬운 말로 고칠 점 알려주기", "수련품새 모드 설명 문구 존재");
assertContains(webHtml, "수정 우선순위로 점검", "경기품새 모드 설명 문구 존재");
assertContains(webHtml, "초등학생도 알 수 있는 쉬운 말", "수련품새는 점수 없이 쉬운 말 안내 중심");
...
assertContains(readUtf8(path.join(root, "www", "service-worker.js")), "poomsae-training-v38-jitae-poomsae-line", "www 직접 배포용 오프라인 엔진 캐시 적용");
...
assertContains(readUtf8(rootServiceWorkerPath), "poomsae-training-root-offline-engine-v7-data-coverage", "최상단 배포용 오프라인 엔진 서비스워커 존재");
...
assertContains(webHtml, "박수 지점 · 잘못됐으면 여기서 고치세요", "준비 구간(박수 지점) 시작 시각도 수정 가능하도록 안내 문구 존재");
assertContains(webHtml, "const lo = i === 0 ? 0 : b[i - 1] + 0.1", "0번 경계(박수 지점) 연쇄 조정 지원");
assertContains(webHtml, "if (!manualEditMode && trimStart > 0", "수동 조정 모드에서는 박수 이전 재생 잠금이 해제되어 직접 확인 가능");
