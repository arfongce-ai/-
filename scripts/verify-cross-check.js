...
assertContains(webHtml, "태권도 품새 수련인을 위한 영상 기반 AI 분석 코치 · v3.32", "헤더 설명 문구 적용");
...
assertContains(webHtml, "active_range_skeleton_valley_prominence_adaptive", "valley prominence 기반 적응형 경계 탐지 적용");
assertContains(webHtml, "const prominenceOf", "valley 깊이(prominence) 계산 함수 존재");
assertContains(webHtml, "refineSegmentBoundaries.lastConfidence", "경계 신뢰도 기록 존재");
assertContains(webHtml, "uniform_fill", "뚜렷한 정지 부족 시 균등분할 안전 폴백 존재");
assertContains(webHtml, "boundary_uniform_filled", "경계 균등 추정 개수 결과 기록");
assertContains(webHtml, "동작명과 영상이 한 칸씩 어긋날 수 있으니", "동작명 밀림 가능성 안내 존재");
assertNotContains(webHtml, "interval * 0.38", "동작 길이를 균등으로 강제하던 ±38% 제약 제거");
