...
assertContains(webHtml, "태권도 품새 수련인을 위한 영상 기반 AI 분석 코치 · v3.21", "헤더 설명 문구 적용");
...
assertContains(webHtml, "boundary-motion-revision-3-", "고밀도 구간 감지 방식과 민감도를 캐시 키에 반영");
assertContains(webHtml, "video.duration * 3", "구간 감지를 초당 3회 수준으로 촘촘하게 수행");
assertContains(webHtml, "high_density_local_motion_minimum_near_expected_boundary", "실제 움직임 최저점을 구간 경계로 우선 선택");
assertContains(webHtml, ".scene-card.priority .scene-shot", "우선 확인 카드의 캡처를 상단 중앙에 배치");
assertContains(webHtml, ".scene-card.competition-full .scene-shot", "경기품새 전체 부분 카드의 캡처를 상단 중앙에 배치");
assertContains(webHtml, "trainingMode === \"competition\" ? \"competition-full\"", "경기품새 전체 부분에 세로형 카드 적용");
