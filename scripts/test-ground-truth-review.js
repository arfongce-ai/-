...
(async () => {
  const reviewPath = path.resolve("data/ground-truth/taegeuk-1-gpt-review-v1.json");
  ...
  assert.equal(calibrationRecordWeight(data.calibration_record), 0.6);
  assert.deepEqual(
    buildTrustedCollectiveProfiles([data.calibration_record]),
    {},
    "single GPT review must not be promoted to a collective profile"
  );
  ...
  assert(Math.abs(mae - 0.591) < 0.001);
  assert(Math.abs(maxError - 1.595) < 0.001);
  assert.equal(within03, 7);
  console.log(`Taegeuk 1 baseline: MAE ${mae.toFixed(3)}s, max ${maxError.toFixed(3)}s, <=0.3s ${within03}/${internalErrors.length}`);
})().catch((error) => {
  console.error(error);
  process.exit(1);
});
